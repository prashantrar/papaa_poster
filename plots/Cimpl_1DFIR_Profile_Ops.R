...
df <- read.csv("Cimpl_1DFIR_Profile_Ops.csv",header=TRUE,sep=",")
pdf("Cimpl_1DFIR_Profile_Ops.pdf",width=4,height=3)

p<-ggplot(data=df,aes(group=factor(Device),color=factor(Device),x=Samples,y=Ops_Per_Cyc))+
  geom_line(size=0.9)+
  geom_point(size=2)+
